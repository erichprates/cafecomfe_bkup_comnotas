...
export function NotesModal({ notes, devotionalTitle, onClose }: NotesModalProps) {
  ...
  return (
    <div className="fixed inset-0" style={{ zIndex: 9999 }}>
      ...
      {/* Alerta de sucesso */}
      <div
        className={`fixed top-0 left-0 right-0 transform transition-transform duration-300 ${
      ...
      {showDeleteConfirm && (
        <div 
          className="fixed top-0 left-0 right-0 transform translate-y-0 transition-transform duration-300 bg-red-500 text-white"
          style={{ zIndex: 10000 }}
        >
      ...
      {/* Modal Content */}
      <div 
        className="fixed inset-4 bg-[#ffffd4] dark:bg-[#2c2c1f] rounded-lg shadow-xl flex flex-col overflow-hidden"
        style={{ zIndex: 9999 }}
      >
